...
def capture_listing_screenshot(url: str, listing_id: str, source: str) -> Optional[str]:
    """
    ...
    try:
        ...
        filename = f"{source}_{listing_id}_{timestamp}.png"
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if source == "ebay":
                    ...
                    try:
                        page.wait_for_selector("#vi-lkhdr", timeout=5000)
                    except PlaywrightTimeoutError:
                        pass
                elif source == "leboncoin":
                    # Wait for main content
                    try:
                        page.wait_for_selector("[data-qa-id='adview_title']", timeout=5000)
                    except PlaywrightTimeoutError:
                        pass
                elif source == "vinted":
                    # Wait for main content
                    try:
                        page.wait_for_selector(".item-details", timeout=5000)
                    except PlaywrightTimeoutError:
                        pass
