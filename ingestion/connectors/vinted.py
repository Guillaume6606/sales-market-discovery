...
class VintedConnector:
    ...
    async def search_items(
        ...
        search_params = {"search_text": keyword, "order": "newest_first"}
    ...
    async def get_item_details(self, item_url: str) -> Listing | None:
        """
    ...
    def _parse_search_results(self, html_content: str) -> list[dict[str, Any]]:
        ...
        for selector in item_selectors:
            ...
            if elements:
                ...
                item_elements = [
                    e for e in elements if e.find("a", href=lambda h: h and "/items/" in h)
                ]
                if item_elements:
                    logger.info(
                        f"Found {len(item_elements)} item containers using selector: {selector}"
                    )
                    break
        ...
        if not item_elements:
            ...
            if item_elements:
                logger.info(
                    f"Found {len(item_elements)} item containers using fallback (link parents)"
                )
    ...
    def _parse_item_element(self, element) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.debug(
                f"Processing item {item_id}, text preview: {element_text[:200] if element_text else 'empty'}"
            )
            ...
            brand_element = element.find(
                ["span", "div"], class_=lambda x: x and "brand" in str(x).lower()
            )
            if brand_element:
            ...
            # Extract size
            size = ""
            size_element = element.find(
                ["span", "div"], class_=lambda x: x and "size" in str(x).lower()
            )
            if size_element:
            ...
            # Extract color
            color = ""
            color_element = element.find(
                ["span", "div"], class_=lambda x: x and "color" in str(x).lower()
            )
            if color_element:
            ...
            if not condition:
                condition_element = element.find(
                    ["span", "div"],
                    class_=lambda x: (
                        x and ("condition" in str(x).lower() or "état" in str(x).lower())
                    ),
                )
            ...
            shipping_element = element.find(
                ["span", "div"],
                class_=lambda x: (
                    x and ("shipping" in str(x).lower() or "livraison" in str(x).lower())
                ),
            )
            if shipping_element:
    ...
    def _parse_structured_data(self, structured_data: dict, item_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Extract brand
            brand = (
                structured_data.get("brand", {}).get("name", "")
    ...
    def _parse_item_html(self, soup: BeautifulSoup, item_url: str) -> dict[str, Any]:
        ...
        brand_element = soup.find(
            ["span", ".brand"],
            class_=lambda x: x and ("brand" in x.lower() or "marque" in x.lower()),
        )
        brand = self.scraping_utils.clean_text(brand_element.get_text()) if brand_element else ""

        # Extract size
        size_element = soup.find(
            ["span", ".size"], class_=lambda x: x and ("size" in x.lower() or "taille" in x.lower())
        )
        size = self.scraping_utils.clean_text(size_element.get_text()) if size_element else ""

        # Extract color
        color_element = soup.find(
            ["span", ".color"],
            class_=lambda x: x and ("color" in x.lower() or "couleur" in x.lower()),
        )
        ...
        condition_element = soup.find(
            ["span", ".condition"],
            class_=lambda x: x and ("condition" in x.lower() or "état" in x.lower()),
        )
        condition = (
            self.scraping_utils.clean_text(condition_element.get_text())
            if condition_element
            else ""
        )

        # Extract price
        price_selectors = [".price", ".item_price", '[class*="price"]']
