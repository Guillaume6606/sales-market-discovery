...
def compute_pmn_confidence(
    sample_size: int, newest_sale_age_days: float, std_dev: float, pmn: float
) -> float:
    """Compute confidence score (0.0 to 1.0) for a PMN estimate."""
    sample_factor = min(sample_size / 30.0, 1.0)
    freshness_factor = max(0.0, 1.0 - newest_sale_age_days / 30.0)
    consistency_factor = max(0.0, min(1.0, 1.0 - std_dev / pmn)) if pmn > 0 else 0.0
    return 0.4 * sample_factor + 0.3 * freshness_factor + 0.3 * consistency_factor
...
DEFAULT_FEES = {"commission": 0.10, "payment": 0.03, "name": "Unknown"}
...
def compute_pmn_for_product(product_id: str, db: Session | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if not product:
            return {"status": "error", "error": "product_not_found", "product_id": product_id}
        ...
        if len(prices) < 10:
            logger.info(
                f"Product {product_id}: Only {len(prices)} sold items, including active listings"
            )
        ...
        if len(prices) < 3:
            logger.warning(
                f"Product {product_id}: Insufficient data for PMN ({len(prices)} prices)"
            )
            return {
        ...
        if timestamps:
            aware_timestamps = [
                t.replace(tzinfo=UTC) if t.tzinfo is None else t for t in timestamps if t
            ]
            if aware_timestamps:
        ...
        confidence = compute_pmn_confidence(
            ...
            newest_sale_age_days=newest_sale_age_days,
            std_dev=std_dev,
            pmn=pmn_result["pmn"],
        )
        ...
        if existing_pmn:
            ...
            existing_pmn.last_computed_at = datetime.now(UTC)
            existing_pmn.methodology = pmn_result["methodology"]
            existing_pmn.confidence = confidence
        else:
            # Create new record
...
def compute_liquidity_score(product_id: str, db: Session | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"liquidity_score": 0.0, "error": str(e)}
    finally:
        if should_close and db:
            db.close()
...
def estimate_margin(
    listing_price: float, pmn: float, shipping_cost: float | None, source: str
) -> dict[str, Any]:
    """
...
def compute_all_product_metrics(
    product_ids: list[str] | None = None, db: Session | None = None
) -> dict[str, Any]:
    """
